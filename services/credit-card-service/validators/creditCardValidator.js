...
const validateCreditCardInput = (data, type) => {
    ...
    if (type === 'application') { 
        // 1. Credit Limit Validation: Within policy bounds [cite: 2461]
        if (!data.requested_limit || data.requested_limit <= 0) {
            errors.push("Requested limit must be a positive number."); 
        }
        
        // 2. Income Validation: Must meet minimum threshold [cite: 2461, 2463]
        if (!data.annual_income || data.annual_income < 300000) {
            errors.push("Income threshold not met."); 
        }
        
        // 3. Employment Type Validation: Required for risk category [cite: 2461]
    ...
    if (type === 'payment') { 
        // 1. Target Card Identification: Required for cardholder mapping [cite: 2462, 2465]
        if (!data.card_id) {
            errors.push("Card ID is required."); 
        }
        
        // 2. Amount Validation: Must be a positive numeric value [cite: 2462]
        if (!data.payment_amount || data.payment_amount <= 0) {
            errors.push("Payment amount must be positive."); 
        }
        
        // 3. Source Account: Validation of payment bank account [cite: 2462, 2477]
        if (!data.source_account_id) {
            errors.push("Source bank account is required."); 
        }
    }

    // Standardized Success/Failure Output [cite: 2462, 2463]
    return {
